...
class Router(object):
    ...
    def verify_target_contract_registration(self):
        with brownie.reverts("Router: Unauthorized contract"):
            self.router.initiateTradeFromAccount(
                self.option_params,
                {"from": self.owner},
            )
    ...
    def verify_multitoken_router(self):
        ...
        txn = self.router.initiateTradeFromAccount(
            params,
            {"from": self.user_1},
        )
    ...
    def verify_trade_initiation_and_execution(self):
        self.router.setKeeper(self.bot, True)
        ...
        queued_time = self.chain.time()
        user = self.accounts.add()
        signed_price = 396e8
        signature, signature_time = self.get_user_signature(
            params, self.user_1.address, user.private_key
        )
        # Transfer and approve usdc
        self.router.registerAccount(user.address, {"from": self.user_1})
        self.tokenX.transfer(self.user_1, 1000000, {"from": self.owner})
        self.tokenX.approve(self.router, 1000000, {"from": self.user_1})
        trade_params = [
            params,
            self.user_1.address,
            queued_time,
        ...
        self.chain.snapshot()
        txn = self.router.initiateTradeForAccount(
            *trade_params,
            {"from": self.bot},
        )
        assert txn.events["InitiateTrade"]["queueId"] == 0, "Wrong event"
        assert self.router.prevSignatures(signature), "Signature not saved"

        with brownie.reverts("Router: Trade already initiated!"):
            self.router.initiateTradeForAccount(*trade_params, {"from": self.bot})
        self.chain.revert()

        self.chain.snapshot()
        txn = self.router.initiateAndResolve(
            *trade_params[:-1],
            self.get_signature(
                self.tokenX_options.address,
                queued_time,
        ...
        assert txn.events["InitiateTrade"]["queueId"] == 0, "Wrong event"
        assert self.router.prevSignatures(signature), "Signature not saved"
        assert txn.events["OpenTrade"]["queueId"] == 0, "Wrong event"
        ...
        self.chain.mine()
        self.chain.snapshot()
        with brownie.reverts("Router: Invalid signature timestamp"):
            txn = self.router.initiateTradeForAccount(
                *trade_params,
                {"from": self.bot},
            )
        ...
        self.chain.snapshot()
        trade_params[1] = self.user_2
        with brownie.reverts("Router: Invalid user signature"):
            txn = self.router.initiateTradeForAccount(
                *trade_params,
                {"from": self.bot},
            )
    ...
    def complete_flow_test(self):
        ...
        self.verify_trade_initiation_and_execution()
